...
extension Photo {
    ...
    public var thumbnailImage: UIImage? {
        ...
        print("Camera: thumbnail size: \(data.count/1024)")
        return UIImage(data: data)
    }
    public var image: UIImage? {
        guard let data = compressedData else { return nil }
        print("Camera: image size: \(data.count/1024)")
        return UIImage(data: data)
    }
}
...
public class CameraService: NSObject, Identifiable {
        ...
        sessionQueue.async {
            if self.isSessionRunning {
                if self.setupResult == .success {
                    ...
                    for ii in self.session.inputs {
                        print("CAMERA: remove input")
                        self.session.removeInput(ii)
                    }
                    
                    for ii in self.session.outputs {
                        print("CAMERA: remove output")
                        self.session.removeOutput(ii)
                    }
    ...
    /// - Tag: CapturePhoto
    public func capturePhoto( completion: @escaping () -> Void = {} ) {
        /*
...
final class CameraModel: ObservableObject {
    ...
    init() {
        ...
        service.$shouldShowSpinner.sink { [weak self] (val) in
            self?.shouldShowSpinner = val
            print("CAMERA shouldShowSpinner \(val)")
        }
        .store(in: &self.subscriptions)
    }
    ...
    func capturePhoto(completion : @escaping () -> Void) {
        service.capturePhoto(completion: completion)
    }
